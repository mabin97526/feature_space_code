...
if __name__ == '__main__':
    ...
    #model.compose(cube,s=1,t=np.array([0,-1,0]))
    #model.compose(ball,s=1,t=np.array([0,0,0.6]))
    #model.compose(ball,s=1,t=np.array([0,0,-0.6]))
    #model.compose(ball,s=1,t=np.array([0,0,0]))
    #model.compose(ball, s=0.5, t=np.array([0.5, 0, 0]))
    #model.compose(ball, s=1, t=np.array([0, 1, 0]))
    #model.compose(cube, s=1, t=np.array([1, -1, 0]))
    #model.compose(cube, s=1, t=np.array([0, -1, 1]))
    #model.compose(cube, s=1, t=np.array([0, -1, 2]))
    opt.ckpt = 'scratch'
    ...
    '''cube = load_model('trial_cc_chair/checkpoints/ngp_ep0034.pth.pth')
    ship = load_model('trial_cc_ship/checkpoints/ngp_ep0250.pth.pth')
    model.compose(cube, s=1, t=np.array([0, -1.0, 0]))
    #model.compose(ship, s=1, t=np.array([0, 0, 0]))
    
    [ 1.        0.       -0.030625 -0.00625   0.0925  ]
    [0.      1.      0.125   0.02375 0.19   ]
    [ 0.        1.       -0.205625  0.026875  0.2175  ]
