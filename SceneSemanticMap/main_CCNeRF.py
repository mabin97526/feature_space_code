...
if __name__ == '__main__':
    ...
    if opt.compose:
        ...
        cube = load_model('features_cube/checkpoints/12_0-12_0.pth')
        ball = load_model('features_/checkpoints/12_0-12_0.pth')
        limo = load_model('trial_limo/checkpoints/12_0-12_0.pth')
        #model.compose(cube, s=0.4, t = np.array([0,0.5,0]))
        #model.compose(ball, s=1,t=np.array([0,0,1]))
        model.compose(limo, s= 1, t = np.array([0,0,1]))
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            if opt.compose:
                trainer.test(test_loader, save_path=os.path.join(opt.workspace, 'compose'),write_video=False)
