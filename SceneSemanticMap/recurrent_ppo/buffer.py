...
class Buffer():
    ...
    def __init__(self, config:dict, observation_space:spaces.Box, action_space_shape:tuple, device:torch.device) -> None:
        ...
        if self.continuous:
            self.actions = torch.zeros((self.n_workers,self.worker_steps,action_space_shape[0]),dtype=torch.float)
            self.log_probs = torch.zeros((self.n_workers, self.worker_steps, action_space_shape[0]))
        else:
        ...
        if config["environment"]["type"]=="UnityCarEnv" and not config["use_compose"]:
            self.obs = torch.zeros((self.n_workers,self.worker_steps)+observation_space[0].shape)
        elif config["environment"]["type"]=="UnityCarEnv" and config["use_compose"]:
            self.obs = torch.zeros((self.n_workers,self.worker_steps)+(84,84,3))
        elif config["environment"]["type"]=="UnityCarRace":
            self.obs = torch.zeros((self.n_workers,self.worker_steps)+observation_space[0].shape)
        else:
