...
def worker_process(remote: multiprocessing.connection.Connection, config:dict,worker_id=1) -> None:
    ...
    try:
        env = create_env(config,worker_id=worker_id)
...
class Worker:
    ...
    def __init__(self, env_config:dict,worker_id=1):
        """
        Arguments:
