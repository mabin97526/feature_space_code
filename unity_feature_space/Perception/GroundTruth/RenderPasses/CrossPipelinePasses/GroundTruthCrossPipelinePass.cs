...
namespace UnityEngine.Perception.GroundTruth
{
    abstract public class GroundTruthCrossPipelinePass : IGroundTruthGenerator
    {
        ...
        protected static void DrawRendererList(
            ScriptableRenderContext renderContext, CommandBuffer cmd, RendererList rendererList, RendererListDesc desc)
        {
            ...
            var descParams = RendererListDesc.ConvertToParameters(desc);

            if (desc.stateBlock == null)
                renderContext.DrawRenderers(
                    descParams.cullingResults, ref descParams.drawSettings, ref descParams.filteringSettings);
            else
            {
                var renderStateBlock = desc.stateBlock.Value;
                renderContext.DrawRenderers(
                    descParams.cullingResults,
                    ref descParams.drawSettings,
                    ref descParams.filteringSettings,
