...
class UnityBaseEnv(gym.Env):
    def __init__(self,file_name:str,worker_id:int,time_scale:int=1,max_steps=2500):
        ...
        self._env = UnityToGymWrapper(Unity_env,uint8_visual=True,flatten_branched=True,allow_multiple_obs=True,action_space_seed=1456)
        ...
        #self.rgb_img = np.zeros((84, 84, 3), dtype=np.uint8)
    ...
    def observation_space(self):
        ...
        obs_1 = spaces.Box(low=0,high=255,shape=(84,84,1),dtype=np.uint8)
        obs_2 = spaces.Box(low=-math.inf,high=math.inf,shape=(802,),dtype=np.float32)
        #obs_3 = spaces.Box(low=-math.inf,high=math.inf,shape=(2,),dtype=np.float32)
    ...
    def reset(self):
        ...
        obs = self._env.reset()
        obs = {"image": obs[0],"ray": obs[1][0:802]}
        return obs,{}
