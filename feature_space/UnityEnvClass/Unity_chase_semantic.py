...
class UnityBaseEnv(gym.Env):
    ...
    def observation_space(self):
        ...
        obs_5 = spaces.Box(low=0, high=255, shape=(256, 256, 3), dtype=np.uint8)
        ...
        obs_shape = gymnasium.spaces.Dict(spaces={"image": obs_1, "semantic":obs_5,"ray":obs_2,"state":obs_4}, seed=42)
        #print(obs_shape)
    ...
    def reset(self):
        self._rewards = []
        obs = self._env.reset()
        extra_info = obs[2][802:810]
        intention = np.zeros(shape=self.intention_shape,dtype=float)
        obs = {"image": obs[0],"semantic":obs[1], "ray": obs[2][0:802],"state":extra_info}
        return obs,{}
    ...
    def step(self,action):
        ...
        reward = float(reward)
        intention = np.zeros(shape=self.intention_shape, dtype=float)
        obs = {"image":obs[0],"semantic":obs[1],"ray":obs[2][0:802], "state":obs[2][802:810]}
...
if __name__ == '__main__':
    env = UnityBaseEnv(file_name="../UnityEnv/CarChase_Semantic/605.exe",worker_id=1)
    state = env.reset()
    print(state)