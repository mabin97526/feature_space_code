...
class UnityBaseEnv(gym.Env):
    ...
    def observation_space(self):
        ...
        obs_2 = spaces.Box(low=-math.inf,high=math.inf,shape=(2,),dtype=np.float32)
        #spaces.Discrete(start=0,n=2)
        #obs_2 = spaces.MultiDiscrete([2,2])
        #obs_shape = gymnasium.spaces.Tuple((obs_1,obs_2),seed=42)
        obs_shape = gymnasium.spaces.Dict(spaces={"image": obs_1, "traffic_lights":obs_2}, seed=42)
        #print(obs_shape)
    ...
    def reset(self):
        self._rewards = []
        obs = self._env.reset()
        #obs = {"image": obs[0], "traffic_lights": obs[1]}
        return obs,{}
