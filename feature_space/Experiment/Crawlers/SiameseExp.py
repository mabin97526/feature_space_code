...
if __name__ == '__main__':
    ...
    env_name = "CrawlersSiameseBarlowTwin"
    Algorithm = "PPO"
    new_logger = configure("./loggers/{}_{}_logger".format(Algorithm, env_name) + timestamp,
                           ["stdout", "csv", "tensorboard"])
    env = make_unity_env(UnityEnvClass.Unity_Crawler_env.UnityBaseEnv, n_envs=5,start_index=15, env_kwargs=dict(file_name="../../UnityEnv/Crawler/UnityEnvironment.exe", time_scale=10,max_steps=1000))
    ...
    model = PPO("MultiInputPolicy", env, verbose=1, device="cuda", learning_rate=3e-4, seed=42, gamma=0.99,
                policy_kwargs=policy_kwargs, use_siamese=True,adjust_learning_rate=False)
