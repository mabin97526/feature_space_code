...
import UnityEnvClass.Unity_Multi_input_CarSearchNormal_Env, UnityEnvClass.Unity_Multi_input_CarRace_InfraredEnv,UnityEnvClass.Unity_Multi_input_CarSearch_InfraredEnv
...
if __name__ == '__main__':
    ...
    env_name = "CarSearchInfraredRandom"
    Algorithm = "PPO"
    new_logger = configure("./loggers/{}_{}_logger".format(Algorithm,env_name) + timestamp, ["stdout", "csv", "tensorboard"])
    env = make_unity_env(UnityEnvClass.Unity_Multi_input_CarSearch_InfraredEnv.UnityBaseEnv, n_envs=5, env_kwargs=dict(file_name="../../UnityEnv/CarSearchInfraredRandom/RLEnvironments.exe", time_scale=10,max_steps=1000))
    #REDQ
    ...
    #                      )
    model = PPO("MultiInputPolicy",env,verbose=1,device="cuda",learning_rate=3e-4,seed=4252,gamma=0.99,policy_kwargs=policy_kwargs,use_siamese=True,adjust_learning_rate=True,save_optimizer=False)
