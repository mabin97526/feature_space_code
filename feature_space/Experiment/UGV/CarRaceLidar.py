...
if __name__ == '__main__':
    ...
    env_name = "CarRaceNoLidarSemantic"
    Algorithm = "REDQ"
    new_logger = configure("./loggers/{}_{}_logger".format(Algorithm,env_name) + timestamp, ["stdout", "csv", "tensorboard"])
    env = make_unity_env(UnityEnvClass.Unity_Multi_input_CarSearch_LidarEnv.UnityBaseEnv, n_envs=5,start_index=15, env_kwargs=dict(file_name="../../UnityEnv/CarRaceLidar/RLEnvironments.exe", time_scale=10,max_steps=2000))
    #REDQ
    ...
    #model = TD3("MultiInputPolicy", env, learning_rate=3e-4, learning_starts=4000, batch_size=3000,policy_kwargs=policy_kwargs, buffer_size=20000, train_freq=(1, "episode"),action_noise=action_noise,verbose=1, seed=7728,policy_delay=4,target_policy_noise=0.4)
