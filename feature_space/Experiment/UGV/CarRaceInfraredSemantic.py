import time
import UnityEnvClass.Unity_Multi_input_CarSearch_Infrared_SemanticEnv
...
if __name__ == '__main__':
    ...
    env_name = "CarRaceInfraredSemantic"
    Algorithm = "REDQ"
    new_logger = configure("./loggers/{}_{}_logger".format(Algorithm,env_name) + timestamp, ["stdout", "csv", "tensorboard"])
    env = make_unity_env(UnityEnvClass.Unity_Multi_input_CarSearch_Infrared_SemanticEnv.UnityBaseEnv, n_envs=5, start_index=15,env_kwargs=dict(file_name="../../UnityEnv/CarRaceInfraredSemantic/RLEnvironments.exe", time_scale=10,max_steps=1000))
    ...
    model.learn(total_timesteps=500000, log_interval=4,callback=TensorboardCallback())
    env.close()