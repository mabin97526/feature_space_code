import time
import UnityEnvClass.Unity_Multi_input_CarSearchNormal_Env, UnityEnvClass.Unity_Multi_input_CarRace_InfraredEnv,UnityEnvClass.Unity_Multi_input_CarRace_Visual_Env
...
if __name__ == '__main__':
    ...
    env_name = "CarRaceNoVisualSemantic"
    Algorithm = "REDQ"
    new_logger = configure("./loggers/{}_{}_logger".format(Algorithm,env_name) + timestamp, ["stdout", "csv", "tensorboard"])
    env = make_unity_env(UnityEnvClass.Unity_Multi_input_CarRace_Visual_Env.UnityBaseEnv, n_envs=5, start_index=15,env_kwargs=dict(file_name="../../UnityEnv/CarRaceVisual/RLEnvironments.exe", time_scale=10,max_steps=1000))
    #REDQ
