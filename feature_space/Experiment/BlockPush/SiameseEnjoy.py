...
if __name__ == '__main__':
    ...
    env_name = "PushBlockSiamese"
    Algorithm = "PPO"
    new_logger = configure("./loggers/{}_{}_logger".format(Algorithm, env_name) + timestamp,
                           ["stdout", "csv", "tensorboard"])
    env = make_unity_env(UnityEnvClass.Unity_PushBlock_env.UnityBaseEnv, n_envs=1,start_index=0, env_kwargs=dict(file_name=None, time_scale=1,max_steps=1000))
    ...
    model.set_parameters(load_path_or_dict="results/VICReg.zip")
    times = 20
    episode_rewards = []
    episode_lengths = []
    while times > 0:
