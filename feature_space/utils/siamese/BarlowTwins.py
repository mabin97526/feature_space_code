...
class BarlowTwinsFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self,observation_spaces:spaces.Dict,lambd,ckpt=None):
        super().__init__(observation_spaces,features_dim=1)
        self.model = BarlowTwins(observation_spaces,lambd)
        param_weights = []
        param_biases = []
        ...
        self.optimizer = LARS(parameters,lr=0,weight_decay=1e-6,
...
class BarlowTwins(nn.Module):
    def __init__(self, observation_spaces: spaces.Dict, lambd:float):
        super().__init__()
        extractors = {}
        projectors = {}
        bns = {}
        ...
        self.bns = nn.ModuleDict(bns)

    def forward(self, observations):
        encoded_tensor_list = []
        for key, obs in observations.items():
            if str(key).__contains__("random"):
                continue
    ...
    def compute_loss(self,observation,observation_):
        ...
        for key,obs in observation.items():
            if key in set(it[0] for it in self.projectors.items()):
                ...
                c = self.bns[key](z1).T @ self.bns[key](z2)
