...
if __name__ == '__main__':
    encoder = RNNEncoder(4,128,64,1)
    obs = th.ones((1,6))
    h = th.ones((1,128))
    output, new_hidden, new_hidden_out = encoder(obs,h)
    decoder = Action_Latent_Decoder(64,128,1,2,2)
    #input = th.ones((1,2,16,64))
    pre_latent = th.ones((1,64))
    hidden = th.ones(1,128)

    o1,a1,a2,hidden = decoder(new_hidden_out,pre_latent,hidden)
    action1 = th.argmax(a1,dim=1)
    action2 = th.argmax(a2,dim=1)
    print(action1)
    print(action2)