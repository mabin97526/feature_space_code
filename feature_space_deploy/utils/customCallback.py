...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        '''if(self.interval%10==0):
            action_0 = self.locals['actions'][0][0]
            action_1 = self.locals['actions'][0][1]
            self.logger.record("action_0",action_0)
            self.logger.record("action_1",action_1)
            self.logger.dump(self.num_timesteps)
            self.interval = 0;
        self.interval += 1;'''
        if(self.locals['dones'][0]):
            e_rewards = self.locals['infos'][0]['reward']
            e_length = self.locals['infos'][0]['length']
            self.logger.record("episode_reward",e_rewards)
            self.logger.record("episode_length",e_length)
            if self.record_intention:
