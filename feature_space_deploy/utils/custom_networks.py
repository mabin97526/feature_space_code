...
if __name__ == '__main__':
    ...
    '''class HyperParams(MULTModel.DefaultHyperParams):
        ...
        output_dim = 64
        all_steps = True
    model = MULTModel(obs_shape,HyperParams).to("cuda")    #out_shape = (1,1,64)
    #model = TensorFusion(obs_shape)
    obs1 = torch.ones((1,12,84,84),dtype=torch.float32).to("cuda")
    obs2 = torch.ones((1,1600),dtype=torch.float32).to("cuda")
