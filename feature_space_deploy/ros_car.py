...
def resize_camera(im_np):
    im_pil = PILImage.fromarray(im_np).resize((416, 416))
...
class RosCar:
    ...
    def __init__(self, ROS_MASTER_URI=None, ROS_IP=None, motor_limit=0.25, repeat_action=False):
        """
        Args:
        ...
        rospy.Subscriber('/camera/rgb/image_raw',
                         numpy_msg(Image), self._camera_callback)
    ...
    def _get_processed_laser_scan(self):
        ...
        if size_delta > 0:  # 如果真实的射线数较少
            ...
            idx = np.concatenate([np.arange(math.floor(-size_delta / 2)), -(np.arange(math.ceil(-size_delta / 2)) + 1)])
    ...
    def get_rl_obs_list(self):
        ...
        rl_camera_obs = np.array(resize_camera(ros_camera) / 255.).astype(np.float32)
        ...
        rl_laser_scan = [ 1.0 if d > 1.0 else d for d in ros_laser_scan ]
...
if __name__ == "__main__":
    ...
    while not car.is_shutdown():
        ...
        # print("image",state[0][0])
        state = state[1][0]
        change_state = state[0:202]
        speed = (1-(-1)) * np.random.random() + (-1)
        angele = (1-(-1)) * np.random.random() + (-1)
        # print("speed",speed)
        car.move_car(speed, angele)
        # print("ray", change_state)
        # print("raylength111111111111",len(change_state))
        # print("raylenth1", len(state[1][0]))
        # print("vector", state[2][0])

        # action = [[0,-1]]  # action is 2 dim
        # state, reward, done, _ = car.env_step(action)
